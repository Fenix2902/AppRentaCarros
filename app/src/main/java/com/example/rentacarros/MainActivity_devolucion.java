...
public class MainActivity_devolucion extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_devolucion_vehiculo);
        ...
        cerrar_devolucion = findViewById(R.id.btn_CerrarSesion);
        ...
        devolucion_devolucion.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!numero_devolucion.getText().toString().isEmpty() && !fecha_devolucion.getText().toString().isEmpty() && !numero_renta_devolucion.getSelectedItem().toString().isEmpty()){
                    db.collection("Vehiculos").whereEqualTo("estado", isChecked).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                        @Override
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            if (task.isSuccessful()) {
                                if (task.getResult().size() > 0) {
                                    ...
                                    db.collection("Rentas").whereEqualTo("renta", numero_renta_devolucion.getSelectedItem().toString()).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                        @Override
                                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                            if (task.isSuccessful()) {
                                                        ...
                                                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                            if(task.isSuccessful()){
                                                                if (task.getResult().isEmpty()) {
                                                                    ...
                                                                    db.collection("Devolucion").add(devolucion_tabla).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                                                                        @Override
        ...
        db.collection("Rentas").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    List<String> dataList = new ArrayList<>();
                    dataList.add("Selecciona una opción");
                    for (QueryDocumentSnapshot document : task.getResult()) {
                        String data = document.getString("renta");
                        dataList.add(data);
                    }
